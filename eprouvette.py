...
class Eprouvette:
    ...
    def __init__(self, doses: List[Any]):
        if len(doses) > self.MAX_DOSES:
            raise EprouvetteError(f"{doses} : On ne peut pas avoir plus de {self.MAX_DOSES} dans l'éprouvette")
    ...
    def __getitem__(self, index: int) -> Any | None:
        ...
        if index >= self.MAX_DOSES:
            raise EprouvetteError(f"Index = {index} : On ne peut pas avoir plus de {self.MAX_DOSES} dans l'éprouvette")
    ...
    def pop_dose(self) -> Any:
        ...
        if self.is_vide:
            raise EprouvetteError("On ne peut pas retirer une dose d'une éprouvette vide")
